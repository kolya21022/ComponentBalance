...
{
    public partial class AddDetailInReleaseUserControl
    {
        public AddDetailInReleaseUserControl()
        {
            InitializeComponent();
            FontSize = Visual.FontSize;
        }

        private void ReleaseProductTextBox_OnPreviewKeyUp(object senderIsTextBox, KeyEventArgs eventArgs)
        {
            // TextBox поиска
        ...
        private void ReleaseProductSearchDataGrid_OnPreviewKeyUp(object senderIsDataGrid, KeyEventArgs eventArgs)
        {
            ...
            else if (searchDataGrid.Items.Count > 0 && eventArgs.Key == Key.Enter)
            {
                ...
                if (selectedItem.BaseType == typeof(ReleaseProduct))
                {
                    var editReleaseProduct = (ReleaseProduct)rawSelectedItem;
                    var viewModel = (AddDetailInReleaseViewModel)DataContext;

                    viewModel.SelectedReleaseProduct = viewModel.ReleaseProductsList.FirstOrDefault(i => i.Id == editReleaseProduct.Id);
                }
            }
        }
        ...
        private void ReleaseProductSearchDataGrid_OnPreviewMouseDown(object senderIsDataGrid, MouseButtonEventArgs e)
        {
        ...
        private void DetailInSearchDataGrid_OnPreviewKeyUp(object senderIsDataGrid, KeyEventArgs eventArgs)
        {
            const int startOfListIndex = 0;
            // DataGrid поиска сущности
            var searchDataGrid = senderIsDataGrid as DataGrid;
            ...
            else if (searchDataGrid.Items.Count > 0 && eventArgs.Key == Key.Enter)
            {
                ...
                {
                    var editDetail = (Detail)rawSelectedItem;
                    var viewModel = (AddDetailInReleaseViewModel)DataContext;

                    viewModel.SelectedDetail = viewModel.DetailsList.FirstOrDefault(i => i.Id == editDetail.Id);
                }
            }
        }

        private void DetailInSearchDataGrid_OnPreviewMouseDown(object senderIsDataGrid, MouseButtonEventArgs e)
        {
            // DataGrid поиска сущности
            var searchDataGrid = senderIsDataGrid as DataGrid;
            if (searchDataGrid == null)
