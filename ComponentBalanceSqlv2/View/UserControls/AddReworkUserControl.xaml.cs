...
{
    public partial class AddReworkUserControl
    {
        public AddReworkUserControl()
        {
            InitializeComponent();
        }
        ...
        private void SearchDataGrid_OnPreviewKeyUp(object senderIsDataGrid, KeyEventArgs eventArgs)
        {
            ...
            else if (searchDataGrid.Items.Count > 0 && eventArgs.Key == Key.Enter)
            {
                ...
                if (selectedItem.BaseType == typeof(Product))
                {
                    ...
                    var viewModel = (AddReworkViewModel)DataContext;
                    viewModel.SelectedProduct = viewModel.Products.FirstOrDefault(i => i.Id == item.Id);
